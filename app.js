...
async function sbFetch(path, opts){
  ...
  try{
    ...
    if(!res.ok){
      ...
      var errMsg = (errBody && (errBody.message||errBody.error)) ? (errBody.message||errBody.error) : ('HTTP ' + res.status);
...
function saveUsers(u){ _usersCache = u; }
...
async function showListings(cat){
  ...
  if(!_fetchOk && !_hasAny){
    ...
    if(_lg2) _lg2.innerHTML = '<div class="no-listings"><div class="nl-ico">📡</div>' +
      ...
      '<button onclick="showListings(\'' + cat + '\')" style="background:#1a2f5e;color:#fff;border:none;padding:10px 28px;border-radius:50px;font-family:Cairo,sans-serif;font-size:14px;cursor:pointer">🔄 إعادة المحاولة</button></div>';
    return;
  }
  renderListings(cat);
}
...
async function confirmSend(){
  ...
  try{
    ...
    // أضف الإعلان للـ cache محلياً فوراً حتى يظهر في "إعلاناتي" بدون انتظار
    ...
    (async function(){
      try{
        ...
        _dbCacheTime = 0;
        try{ await loadDBAsync(true); } catch(e){ console.warn('loadDBAsync bg:', e.message); }
        updateNotifDot();
        updateMyAdsCard();
      ...
      } catch(e){ console.error('confirmSend bg DB error:', e.message); }
    })();
  } catch(e){
  ...
  try{
    ...
    (async function(){
      try{
        ...
        var _ms=document.getElementById('s-myads'); if(_ms&&_ms.classList.contains('active')) _doRenderMyAds();
        var _ao=document.getElementById('admin-overlay'); if(_ao&&!_ao.classList.contains('hidden')) renderAdminPanel();
      } catch(e){ console.error('submitAdDirect bg DB error:', e.message); }
    })();
  } catch(err){
...
function toggleBlockUser(phone){
  ...
  u.blocked=!u.blocked;
  saveUsers(users);
  saveUserToSB(u).catch(function(e){ console.error('toggleBlockUser SB error:', e.message); });
  renderUsersPanel();
...
function clearAllData(){
  ...
  ['fares_ads','fares_views','fares_favs','fares_users','fares_ad_counter',
   'fares_current_user','fares_broker_profile','fares_saved_phones','fares_settings'].forEach(function(k){
...
function resetForm(){
  ...
  // Reset featured
  featuredEnabled=false; transferPhoto=null;
...
window.addEventListener('load', async function(){
  ...
  if(isStandalone){
    ...
    if(savedUser && savedUser.phone){
      ...
      updateUserBadge();
      if(savedUser.isAdmin) document.getElementById('admin-fab').style.display = 'flex';
      updateBrokerHomeCard();
      toast('🎉 مرحباً بعودتك!');
      startRealtimeSync();
    } else {
...
async function saveUserProfileToSB(profile){
  try{
    ...
    if(!_pres.ok){ var _pe=null; try{_pe=await _pres.json();}catch(e){} console.error('saveUserProfileToSB failed:', _pres.status, _pe); }
  } catch(e){}
}
...
function updateBrokerHomeCard(){
  ...
  if(profile){
    ...
    // لا نُخفي ownerZone هنا — updateMyAdsCard هي المسؤولة عن إظهاره/إخفائه
...
function logoutBroker(){
  ...
  // Hard session reset - clear all user data
  var savedPhones = localStorage.getItem('fares_saved_phones'); // احتفظ بالأرقام المحفوظة
  localStorage.clear();
...
function adminToggleFeaturedFromListing(id){
  ...
  showListings(_currentListingsCat||'');
...
function renderPropModal(ad){
  ...
  var brokerHtml = '<div class="prop-modal-section">'+
    ...
    '<div class="prop-broker-card" onclick="openBrokerAds(\''+sBPhone+'\',\''+sBName.replace(/\x27/g,'%27')+'\')">' +
    ...
    '</div>'+
    '<button class="prop-action-broker-ads" onclick="openBrokerAds(\''+sBPhone+'\',\''+sBName.replace(/\x27/g,'%27')+'\')" >'+
